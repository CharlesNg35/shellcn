import { Severity, type Severity as SeverityValue } from "@/types/projection";
...
const BADGE_SEVERITY: Record<SeverityValue, string> = {
  [Severity.Success]:
...
export function badgeClassFor(
  severities: Record<string, SeverityValue> | undefined,
  value: unknown,
): string {
  const sev = severities?.[String(value ?? "").toLowerCase()];
  return BADGE_SEVERITY[sev ?? Severity.Secondary];
}
