...
export async function redeemInvite(payload: RedeemInvitePayload): Promise<InviteRedeemResponse> {
  const response = await apiClient.post<ApiResponse<InviteRedeemResponse>>(
    AUTH_INVITE_ACCEPT_ENDPOINT,
    payload
  )
  return unwrapResponse(response)
}
