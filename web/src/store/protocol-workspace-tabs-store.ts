...
const DEFAULT_FILES_TITLE = 'Files'
const DEFAULT_COLUMNS = 1

export type WorkspaceViewType = 'terminal' | 'sftp' | 'files'
...
export interface ProtocolWorkspaceTabsStore {
...
const TAB_ORDER_KEY_PREFIX = 'workspace.tabOrder.'
...
export const useProtocolWorkspaceTabsStore = create<ProtocolWorkspaceTabsStore>()((set, get) => ({
  sessions: {},
  orderedSessionIds: [],
  activeSessionId: null,
  openSession: ({ sessionId, connectionId, connectionName, protocolId }) => {
  ...
  ensureTab: (sessionId, type, options) => {
    ...
    const resolvedType = type === 'files' ? 'sftp' : type
    const tabId = makeTabId(trimmed, resolvedType)
    ...
    const tab: WorkspaceTab = {
      ...
      title:
        options?.title ??
        (type === 'files' || resolvedType === 'sftp'
          ? DEFAULT_FILES_TITLE
          : DEFAULT_TERMINAL_TITLE),
      closable: options?.closable ?? resolvedType !== 'terminal',
      meta: options?.meta,
    }
...
export const useSshWorkspaceTabsStore = useProtocolWorkspaceTabsStore

export function openSessionFromRecord(record: ActiveConnectionSession) {
  const store = useProtocolWorkspaceTabsStore.getState()
...
export function resetProtocolWorkspaceTabsStore() {
  useProtocolWorkspaceTabsStore.getState().reset()
}

export const resetSshWorkspaceTabsStore = resetProtocolWorkspaceTabsStore

export const selectSessionWorkspace =
  (sessionId: string) =>
  (state: ProtocolWorkspaceTabsStore): SessionWorkspaceState | undefined =>
