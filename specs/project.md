...
type ActionEffect struct {
    ...
    OpenPanel     *OpenPanelEffect     // when Type == "open_panel"
}
...
// OpenPanelEffect opens a panel after the action succeeds; the source params may
// interpolate the action's JSON result via ${response.x}, so a route's output
// drives a follow-up panel (create a resource, then open a terminal into it).
type OpenPanelEffect struct {
    Open   OpenTarget // dock | dialog
    Panel  PanelType
    Title  string
    Source *DataSource
    Config PanelConfig
...
type StreamKind string // terminal, logs, query, desktop, metrics, task, canvas, resource
type Stream struct {
...
hidden background exec. The `open_panel` effect opens a dock/dialog panel whose
source params interpolate the action's JSON result (`${response.x}`), expressing
multi-step flows declaratively — e.g. a "Debug" action POSTs a route that adds a
pod's ephemeral container and returns its name, then `open_panel` opens an exec
terminal with `container=${response.container}`. No bespoke server glue, and the
container is only created on an explicit action (not on viewing a tab).

**Live resource streams.** `StreamResource` is a server-push watch of resource
state: list deltas for a table/tree (the existing `ResourceEvent` shape) or
current-object snapshots for a detail/editor. Panel configs opt in with a `Watch
*DataSource` (on `ObjectDetailConfig`, `CodeEditorConfig`, `TimelineConfig`)
pointing at a `StreamResource` WS route; the generic renderer keeps the panel
fresh without polling. A `CodeEditor` reconciles non-destructively — live-updates
when clean, surfaces a "changed on server" notice when there are unsaved edits —
and uses `RefreshField` to reset its baseline to the server's canonical content
after save (and `DryRunKey` to preview the would-be result as a diff). The core
treats `StreamResource` like a log stream for keepalive (server-push, no
continuous client reader). A plugin backs single-object watches cheaply with a
field selector and may multiplex many subscribers onto one upstream watch.
...
type Condition struct {
    AllOf []Rule      // AND of leaf rules
    AnyOf []Rule      // OR of leaf rules
    All   []Condition // AND of nested groups
    Any   []Condition // OR of nested groups
    Not   *Condition  // negation
}
type Operator string // eq, neq, in, nin, empty, notEmpty, gt, lt, gte, lte, contains
type Rule struct {
    Field string // dotted path, e.g. "can.delete" → record.can.delete (exact key wins)
    Op    Operator
    Value any
}
```

`Condition` composes arbitrary boolean logic — `All`/`Any`/`Not` nest sub-conditions
so e.g. `(A and B) or (C and not D)` is expressible without precomputing a boolean
server-side. `Rule.Field` resolves a **dotted path** (`can.delete` reads
`record.can.delete`); an exact flat key wins first, so existing flat conditions are
unchanged. The numeric (`gt`/`lt`/`gte`/`lte`) and `contains` operators round out
`eq`/`in`/`empty`. Both the Go (server-side field VisibleWhen) and the browser
renderer share one evaluator, so the same condition behaves identically wherever it
runs. Example: a k8s detail attaches a `can` map from a `SelfSubjectAccessReview`
and gates destructive actions with `EnabledWhen {field: "can.delete", op: eq, value: true}`
(absent → fails open, never hiding an action the user might be able to perform).
