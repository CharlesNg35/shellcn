...
type CredentialKind string // ssh_private_key, ssh_password, tls_client_cert, db_password, api_token, ...
...
`kind`, optional `username`, `protocols`, timestamps). The response never
...
It binds to two routes via panel `Config`:

```go
// Tab.Config for a file_browser panel
// {
//   "listRouteId": "<defaults to Tab.Source.routeId>", // dir listing → Page[FileEntry]
//   "readRouteId": "sftp.read",   // GET file content/bytes for preview/download
//   "pathParam":   "path",        // the route param that carries the directory/file path
//   "writable":    true            // gates upload/rename/delete affordances (optional)
// }
```
...
`tls_client_cert`, `db_password`, `api_token`, ...), non-secret metadata
(display name, optional username, allowed protocols), and encrypted secret material
in the vault. A
...
internal/
  domain/      pure types + interfaces (no deps): User, Connection, Credential, Grant, AuditEntry…
  store/       GORM-backed repositories behind UserStore/ConnectionStore/CredentialStore/AuditStore… (§11.1)
...
- **M1 — Core runtime:** plugin registry + manifest validation + real browser
  projection; connection CRUD + reusable credential CRUD + encrypted secrets;
