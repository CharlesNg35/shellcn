...
5. **One connection, many capabilities, many channels.** A single authenticated
   session is multiplexed: an SSH session yields a terminal, SFTP, tunnels, and
...
| Term           | Meaning                                                                                                                                                     |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Connection** | Stored config describing _how to authenticate/connect_ to one target. Owned by a user, optionally shared.                                                   |
...
| Connection storage + secret encryption                            | Config **schema** (field shapes only)    |
...
affordances — permission and risk are never re-declared (single source of truth).
...
stream types, panel configs, and `DataSource` bindings (RouteIDs only). It
...
type FieldType string // text, number, password, select, multiselect,
                      // file, toggle, textarea, json, duration
...
logs/audit, never serialize back to the client.
...
The browser holds opaque `RouteID`s + a params map (`DataSource.Params`, §6.2).
...
- **List controls** use reserved top-level keys: `cursor`, `limit`, `filter`,
  `sort` (§7.2).
- **Request body** (POST/PUT/PATCH) is JSON, validated against `Route.Input` via
  `rc.Bind` (§5.4).
...
4. The user runs it on the target. The agent dials the global
   ...
   the token for a **long-lived, connection-scoped** credential and registers its
   tunnel.
...
long-lived credential is scoped to exactly one connection and one `ProxyTarget`
...
- **SSH:** strict host-key verification; "ignore host key" is never the default.
  First connection / changed key is handled by **`known_hosts` import** plus a
  **TOFU approval** flow — surfaced as a generic **connection verification
  challenge** (see below), not SSH-specific frontend code.
...
**Connection verification challenges (generic).** When `Connect` needs a human
decision before proceeding (unknown/changed SSH host key, an untrusted TLS cert,
an MFA prompt), it returns a typed `VerificationRequired` result carrying a
structured payload — `kind` (e.g. `host_key`), a human message, and the data to
show (fingerprint, old/new key). The core surfaces it; the frontend renders a
**standard confirm/decision panel** and posts the user's accept/reject back. On
accept, the plugin persists the decision (e.g. appends to `known_hosts`). This
keeps host-key/cert/MFA prompts **fully data-driven** — no per-plugin UI (§12).
...
- `ent` (entgo.io) — typed, cross-database data access for the platform store
  (§11.1); wrapped behind repository interfaces
- pure-Go SQL drivers (no CGO ⇒ single-binary holds): `modernc.org/sqlite`
  (default, embedded — `.gitignore` already targets `*.db`), `jackc/pgx`
  (Postgres), `go-sql-driver/mysql` (MySQL/MariaDB). `pgx` also serves the
  PostgreSQL plugin (LISTEN/NOTIFY, COPY).
...
connections + encrypted secrets, audit log, snippets, session/agent metadata,
preferences). It is **not** the database _plugins_ (`postgresql`, `mysql`, …) manage
_remote_ databases and are unrelated to how ShellCN persists its own state).
...
- **Data access: `ent`, not GORM.** `ent` is schema-as-code that generates a
  fully-typed client and emits dialect-correct SQL for SQLite/Postgres/MySQL from
  **one** schema — true cross-DB without per-engine query duplication — plus
  built-in migration support.
  - _Why not GORM:_ a runtime-reflection, `interface{}`-heavy API with weak
    compile-time safety and surprising implicit behavior (zero-value updates,
    soft-deletes, hooks). Those are unacceptable footguns for a security control
    plane that stores credentials and writes audit records.
  - _Alternative considered:_ `sqlc` (compile-time-safe, transparent
    hand-written SQL) — excellent, but cross-dialect means maintaining per-engine
    query files, which `ent` abstracts away. Choose `sqlc` only if hand-written
    SQL transparency matters more than one-codebase cross-DB.
  - _Escape hatch:_ for hot paths (e.g. audit inserts) or dynamic queries, drop
    to raw SQL via ent's `sql` builder — kept inside `internal/store`.
- **Repository pattern (the DX/structure win).** Nothing outside `internal/store`
  imports `ent`. The app depends on small interfaces — `UserStore`,
  `ConnectionStore`, `AuditStore`, `GrantStore`, … — so the engine is swappable,
  the ORM never leaks, and tests use in-memory fakes (no DB needed for unit tests).
- **V1 migrations are automatic, GORM-style.** On startup the server opens the
  configured store and auto-creates missing tables/indexes plus additive schema
  changes. Destructive changes (drop table/column, rewrite data, delete/truncate
  audit rows) are never automatic; those require an explicit reviewed migration.
  This keeps early development fast while the `internal/store` boundary preserves
  the option to move to reviewed ent/Atlas migrations later.
- **Secrets stay encrypted above the store.** Encryption happens in the service
  layer (§9.3); the store only ever persists opaque ciphertext, independent of
  the engine.
...
1. On open, it fetches `GET /api/plugins/{name}` → layout, tabs/tree, columns,
   actions, streams, panel configs, `DataSource` bindings.
...
```
Layout = LayoutTabs
Tabs   = [Terminal, Files, Tunnels, Snippets]      // all connection-level
Session = sshSession{ client, mu, sftp }           // SFTP reuses the TCP conn
Routes  = ssh.shell(WS,privileged), ssh.sftp.*(safe/write), ssh.tunnel(privileged)
```
...
> is a **file-only** plugin for users who don't want a shell/tunnels: its manifest
...
```
cmd/
  server/      entrypoint; wires dependencies, registers compiled-in plugins
  agent/       shellcn-agent: the plugin-agnostic reverse-tunnel proxy (§8.3)
internal/
  domain/      pure types + interfaces (no deps): User, Connection, Grant, AuditEntry…
  store/       ent-backed repositories behind UserStore/ConnectionStore/AuditStore… (§11.1)
...
- **Layered, dependencies point inward:** `domain` ← `store`/`service` ←
  `server`/transport. `domain` imports nothing; the HTTP layer is a thin adapter.
- **Explicit dependency injection:** constructors (`New(...)`) wired once in
  `cmd/server/main.go`. No globals, no service locator, no `init()` magic beyond
...
- Adding a protocol = one plugin package + one `registry.Register(...)` line.
  **Zero** core changes, **zero** frontend changes.

### 14.3 Tooling

- `golangci-lint` + `gofumpt` + `go vet`; `sqlc`/`ent` codegen via `go generate`.
...
- `Makefile`: `build · test · lint · gen · dev` (Go live-reload via
...
- **M1 — Core runtime:** plugin registry + manifest validation + real browser
  projection; connection CRUD + encrypted secrets; session/channel manager;
  transport (direct dialer + tunnel registry); chi route wrapper + WS tickets;
  ent store (SQLite default, §11.1) + repository interfaces; local auth + Casbin;
  audit. Swap the mock server for the real one — the UI is unchanged.
- **M2 — SSH/SFTP (reference plugin, direct transport):** validates the **real
  terminal + file-browser** panels (first real streaming), tunnels, snippets, plus
  auth, secrets, channel lifecycle, and audit end to end.
- **M3 — Docker (introduces agent transport, L4):** containers/images/volumes/
  networks tree; validates **real logs + exec + watch streams**, inspect,
  start/stop/restart; plus the `shellcn-agent` (`tcp`/`unix` modes) + enrollment
  flow (§8.4). Proves sidebar-tree + resource detail + reverse connectivity +
  privileged-risk handling.
...
- **Agent operations:** versioning/auto-update of `shellcn-agent`, agent↔core
  version skew, credential rotation, and multiple agents for one connection (HA).
