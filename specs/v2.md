...
- **Desktop/graphical recording:** VNC/RFB now and RDP/Guacamole later share a
  platform recording contract. Browser canvas capture (`webm_canvas`) is useful
  but not authoritative for strict compliance; plugins may implement native
  recording providers for protocol-specific authoritative formats.

Admins may list/view all recordings. Normal users may list/view recordings they
created, plus connection-scoped recordings allowed by grant/policy. Recording
read/delete operations are audited separately from the original stream route.
