...
- **Data access: `gorm` (GORM).** A developer-friendly, full-featured ORM with
  **no code-generation step** — models are plain Go structs and the schema is
  kept in sync at runtime via `AutoMigrate`. One model set emits dialect-correct
  SQL for SQLite/Postgres/MySQL, so there is no per-engine query duplication.
...
internal/
  ...
  store/       GORM-backed repositories behind UserStore/ConnectionStore/AuditStore… (§11.1)
...
- **M1 — Core runtime:** plugin registry + manifest validation + real browser
  ...
  GORM store (SQLite default, §11.1) + repository interfaces; local auth + Casbin;
