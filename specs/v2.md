...
7. **Single self-contained binary.** Pure-Go dependencies only, so the frontend
   and datastore embed cleanly. Anything needing an external daemon, such as an
   RDP plugin using `guacd`, is an optional sidecar, not part of the core promise.
...
- **No RDP/SPICE protocol engine in the core binary.** RDP requires an external
  `guacd` sidecar. The core exposes a generic `remote_desktop` panel contract and
  validates the plugin-declared engine instead of hardcoding a protocol.
...
const (
    RemoteDesktopEngineNoVNC     RemoteDesktopEngine = "novnc"     // RFB/VNC stream rendered by noVNC
    RemoteDesktopEngineGuacamole RemoteDesktopEngine = "guacamole" // Guacamole protocol stream, normally backed by guacd
)

type RemoteDesktopConfig struct {
    Engine     RemoteDesktopEngine // required
    Resize     bool
    Clipboard  bool
    Audio      bool
    RepeaterID string
...
`engine: "novnc"` means the route is an RFB/VNC byte stream and the browser lazy
loads noVNC. `engine: "guacamole"` means the route is a Guacamole protocol stream
over WebSocket and the plugin/session is responsible for connecting to `guacd`
or another compatible backend. Both engines use the same `remote_desktop` panel
type; the frontend selects the engine from manifest config and lazy-loads only
that engine's client library.
...
- **RDP needs `guacd`** (Apache Guacamole's C daemon) and therefore ships as an
  optional sidecar-backed plugin. The `remote_desktop` panel remains generic:
  plugins must declare `RemoteDesktopConfig.Engine`, and the validator rejects
  missing or unsupported engines.
...
- **Panels are code-split.** Only a small core is bundled up front (shell + the
  ...
  — `code_editor` (Monaco), `remote_desktop` engines (noVNC now, Guacamole
  client when enabled), `metrics`/`graph`/`trace` (visualization libs),
...
vm.Detail = [Overview(metrics), Console(remote_desktop engine=novnc), Hardware(form),
...
- **Additional desktop engines:** `remote_desktop` is now engine-pluggable with
  typed `novnc` and `guacamole` declarations. vSphere **WebMKS** and SPICE remain
  out of scope until they have maintained browser clients and validator-backed
  engine contracts.
