...
type InstallArtifact struct {
    ...
    Template string // rendered with generic context + funcs: .ConnectURL, .GatewayConnectURL, .Token, .Image, .Insecure, .LocalhostHost, .LocalhostHostRequired, shellquote
...
1. **Create enrollment** (authenticated):
   ...
   ```jsonc
   { "kind": "docker-run",
     "command": "docker run --rm --name shellcn-agent --add-host=host.docker.internal:host-gateway -e SHELLCN_CONNECT_URL=wss://host/api/agent/connect -e SHELLCN_ENROLL_TOKEN=… ghcr.io/charlesng35/shellcn-agent:latest" }
   { "kind": "k8s-manifest",
