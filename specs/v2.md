...
```
Tabs      = [Overview(document), Keys(kv), Console(query_editor), Clients(table),
...
Console   = Redis command console over query_editor with Redis labels/completion,
            read-only and write-confirmation safety gates.
