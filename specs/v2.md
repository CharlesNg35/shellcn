...
`devops`, `observability`, `messaging`, and `other`. The browser receives the
...
type TableConfig struct {
    ...
    // Editable data grid (TablePlus/Beekeeper-class) — plugin-agnostic.
...
The data grid identifies a row either from `RowKey` columns or from a per-row
`_key` map a list route attaches (the SQL plugins emit `_key` from the primary
key; a table with no key stays read-only so the wrong record is never mutated).
Mutation bodies are uniform across every plugin, so the renderer ships zero
per-plugin code; the driver-neutral DML builder lives in `plugins/shared/sqldb`
(`Dialect.Insert/Update/Delete`, parameterized, identifier-validated). The
backend additionally re-validates the supplied key against the table's real
primary key (`sqldb.ValidateRowKey`) and requires exactly one affected row, so a
client can never widen a mutation into a multi-row sweep.
