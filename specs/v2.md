...
- **Session health vs plugin health:** `Session.HealthCheck(ctx)` is the
  per-user live connection probe used for badges and idle lifecycle.
  `plugin.HealthChecker` is an optional global plugin/catalog health hook and
  must not be treated as a user's connection state.
