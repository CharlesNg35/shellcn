...
- **No SPICE.** SPICE has no production-grade browser client, so it stays out of
  scope. (RDP, once deferred to a `guacd` sidecar, is now decoded in-process by
  the pure-Go `grdp` client and bridged to noVNC — see §6.2.) The core exposes a
  generic `remote_desktop` panel contract and validates the plugin-declared engine.
...
loads noVNC. VNC plugins stream raw RFB after the gateway authenticates upstream;
RDP plugins decode the session with the pure-Go `grdp` client and bridge it to a
synthetic RFB stream. Both therefore render through the same `remote_desktop`
panel and noVNC client; the frontend lazy-loads that one engine.
...
- **RDP is pure-Go** via the GPL-licensed `grdp` client: the gateway authenticates
  ...
  stream, so no `guacd` daemon is required. (Adopting `grdp` makes ShellCN GPL-3.0.)
  The `remote_desktop` panel stays generic — plugins declare
  `RemoteDesktopConfig.Engine` and the validator rejects unsupported engines.
