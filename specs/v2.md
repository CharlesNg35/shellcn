...
labels of the parent nodes you navigated through (root group excluded). This is
automatic for any nested tree the plugin already declares (`db / schema` for a
SQL table) — **no per-plugin qualifier field, zero extra wiring**. For items
opened _outside_ the tree (a cross-link cell, a detail's sub-table) the renderer
falls back to `Scope`/`Namespace` — which exist anyway as the resource's intrinsic
identity (interpolated as `${resource.scope}`/`${resource.namespace}` for data),
so this is not qualifier-specific stamping. A flat, globally-unique kind (Kafka
topics, Prometheus metrics) has neither an ancestor path nor scope/namespace and
correctly shows the bare `Name`.
