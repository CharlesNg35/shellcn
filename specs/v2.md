...
type Action struct {
    ...
    Open        OpenTarget // "view" (default, run the route) | "dock" | "dialog"
...
**`Scope`/`Namespace` double as the generic disambiguator.** The renderer can't
infer a resource's parent — only the plugin knows a Postgres table lives in a
schema, a Docker container belongs to a compose project, or a Podman container
lives in a pod. So a plugin **populates `Scope`/`Namespace` with the resource's
parent hierarchy**, and the generic renderer uses them everywhere it needs to
distinguish same-named resources — most visibly as the dim qualifier on workbench
tabs (`scope / namespace`), so two tables both named `users`, or two `redis`
containers in different projects, stay distinct. There is **zero per-plugin UI**:
a plugin that omits both fields (a flat, globally-unique namespace like Kafka
topics or Prometheus metrics) simply renders the bare `Name` — correct, not
broken. Use them whenever a real parent exists; leave them empty when a resource
is top-level or its `Name` is already self-qualifying.
...
- `_links` — map of column key → `ResourceRef`; the grid turns those cells into
  links that open the related resource. The renderer doesn't know or care *why*
...
type TreeGroup struct {
    ...
    ResourceKind string     // optional: group click opens this resource list
