...
type CredentialKindInfo struct {
    ...
    CompatibleProtocols []string // empty = no kind-level restriction
}
...
`GET /api/credential-kinds` catalog. Kinds define labels, secret input shape, an
optional identity/principal label, and compatible protocols. A plugin still owns
the stricter selector on each `credential_ref` field, so an SSH field only lists
SSH-compatible credentials and never shows unrelated kinds such as kubeconfig.
