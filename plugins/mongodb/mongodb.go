...
func (p *Plugin) Manifest() plugin.Manifest {
	return plugin.Manifest{
		...
		Version:             "0.1.0",
		Title:               "MongoDB",
		Description:         "MongoDB cockpit with databases, collections, documents, indexes, stats, and command console.",
		Icon:                plugin.Icon{Type: plugin.IconSVG, Value: mongodbSvgIcon},
		Category:            plugin.CategoryDatabases,
		Config:              configSchema(),
		Capabilities:        []plugin.Capability{"documents", "collections", "query_editor", "indexes"},
...
func documentResource() plugin.ResourceType {
	return plugin.ResourceType{
		...
		Detail: plugin.DetailView{
			...
			DefaultTab: "document",
			Tabs: []plugin.Panel{
				{Key: "document", Label: "Document", Icon: icon("braces"), Type: plugin.PanelDocument, Source: &plugin.DataSource{RouteID: "mongodb.document.read", Params: map[string]string{"id": "${resource.uid}"}}},
...
func actions() []plugin.Action {
	return []plugin.Action{
		...
		{ID: "mongodb.index.create", Label: "Create index", Icon: icon("plus"), RouteID: "mongodb.index.create", Params: collectionParams(), OnSuccess: &plugin.ActionSuccess{SelectTab: "indexes"}},
		{ID: "mongodb.index.drop", Label: "Drop index", Icon: icon("trash-2"), RouteID: "mongodb.index.drop", Params: map[string]string{"database": "${record.database}", "collection": "${record.collection}", "name": "${record.name}"}, Confirm: true, ConfirmText: "Drop this index?", OnSuccess: &plugin.ActionSuccess{SelectTab: "indexes"}, Bulk: true},
		{ID: "mongodb.collection.drop", Label: "Drop collection", Icon: icon("trash-2"), RouteID: "mongodb.collection.drop", Params: collectionParams(), Confirm: true, ConfirmText: "Drop this collection and every document in it?", OnSuccess: &plugin.ActionSuccess{Navigate: plugin.NavigateList}, Bulk: true},
		{ID: "mongodb.document.create", Label: "Create document", Icon: icon("plus"), RouteID: "mongodb.document.create", Params: collectionParams(), Open: plugin.OpenDialog, Panel: plugin.PanelCodeEditor, Config: plugin.CodeEditorConfig{Language: "json", InitialContent: "{\n  \"name\": \"value\"\n}", SaveRouteID: "mongodb.document.create", SaveMethod: plugin.MethodPost, SaveParams: collectionParams(), SaveBodyKey: "document"}, OnSuccess: &plugin.ActionSuccess{SelectTab: "documents"}},
		{ID: "mongodb.document.delete", Label: "Delete document", Icon: icon("trash"), RouteID: "mongodb.document.delete", Params: map[string]string{"id": "${resource.uid}"}, Confirm: true, ConfirmText: "Delete this document?", OnSuccess: &plugin.ActionSuccess{Navigate: plugin.NavigateList}, Bulk: true},
	}
}
