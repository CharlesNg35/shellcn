...
func actions() []plugin.Action {
	...
	base := []plugin.Action{
		...
		{ID: "kubernetes.resource.scale", Label: "Scale replicas", Icon: lucide("move-vertical"), RouteID: "kubernetes.resource.scale", Params: uid, EnabledWhen: &plugin.Condition{AllOf: []plugin.Rule{canPatch}}},
		{ID: "kubernetes.resource.restart", Label: "Restart rollout", Icon: lucide("refresh-cw"), RouteID: "kubernetes.resource.restart", Params: uid, Confirm: true, ConfirmText: "Restart this workload by updating its pod template? This starts a rolling replacement of matching Pods.", EnabledWhen: &plugin.Condition{AllOf: []plugin.Rule{canPatch}}},
		{ID: "kubernetes.node.cordon", Label: "Cordon", Icon: lucide("ban"), RouteID: "kubernetes.node.cordon", Params: uid, Confirm: true, ConfirmText: "Mark this node unschedulable?", EnabledWhen: &plugin.Condition{AllOf: []plugin.Rule{{Field: "unschedulable", Op: plugin.OpNeq, Value: true}, canPatch}}, Group: "Scheduling"},
		{ID: "kubernetes.node.uncordon", Label: "Uncordon", Icon: lucide("circle-check"), RouteID: "kubernetes.node.uncordon", Params: uid, EnabledWhen: &plugin.Condition{AllOf: []plugin.Rule{{Field: "unschedulable", Op: plugin.OpEq, Value: true}, canPatch}}, Group: "Scheduling"},
		{ID: "kubernetes.node.drain", Label: "Drain", Icon: lucide("trash-2"), RouteID: "kubernetes.node.drain", Params: uid, Confirm: true, ConfirmText: "Cordon this node and evict eligible Pods? DaemonSet and mirror Pods are skipped; PodDisruptionBudgets can block eviction.", EnabledWhen: &plugin.Condition{AllOf: []plugin.Rule{canPatch}}, Group: "Scheduling"},
		{ID: "kubernetes.rollout.undo", Label: "Undo rollout", Icon: lucide("undo-2"), RouteID: "kubernetes.rollout.undo", Params: uid, Confirm: true, ConfirmText: "Roll back this Deployment to its previous ReplicaSet revision?", EnabledWhen: &plugin.Condition{AllOf: []plugin.Rule{canPatch}}},
		{ID: "kubernetes.cronjob.trigger", Label: "Trigger now", Icon: lucide("play"), RouteID: "kubernetes.cronjob.trigger", Params: uid, Confirm: true, ConfirmText: "Create a one-off Job from this CronJob now? This does not change the schedule.", EnabledWhen: &plugin.Condition{AllOf: []plugin.Rule{canPatch}}},
