...
// In-cluster ServiceAccount mount paths. These are Kubernetes-specific and so
// live here, in the plugin — declared into the otherwise plugin-agnostic agent's
// ProxyTarget. The agent treats them only as "a bearer-token file" and "a CA
// file"; it has no Kubernetes knowledge.
const (
	...
	return []plugin.TreeGroup{
		{
			...
			Source:       plugin.DataSource{RouteID: "kubernetes.namespaces.tree"},
...
func resources() []plugin.ResourceType {
	return []plugin.ResourceType{namespaceResource()}
}
...
func namespaceResource() plugin.ResourceType {
	return plugin.ResourceType{
		Kind:  "namespace",
		Title: "Namespaces",
		List:  plugin.DataSource{RouteID: "kubernetes.namespaces.list"},
			...
			Tabs: []plugin.Tab{
				{
					...
					Icon:   plugin.Icon{Type: plugin.IconLucide, Value: "info"},
					Panel:  plugin.PanelDocument,
					Source: &plugin.DataSource{RouteID: "kubernetes.namespace.get", Params: map[string]string{"name": "${resource.name}"}},
				},
			},
		},
	}
...
// k8sManifestArtifact deploys the agent into the cluster with a ServiceAccount
// bound to cluster-admin (the cockpit acts on the user's behalf — declared
// privileged). The enrollment token travels inside the manifest body (a Secret),
// never in a URL/path. `kubectl apply -f -` reads it from stdin.
const k8sManifestArtifact = `kubectl apply -f - <<'EOF'
apiVersion: v1
kind: Namespace
metadata:
  name: shellcn-agent
