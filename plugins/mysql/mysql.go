...
func actions() []plugin.Action {
	return []plugin.Action{
		...
		{ID: "mysql.column.drop", Label: "Drop column", Icon: icon("trash"), RouteID: "mysql.column.drop", Params: map[string]string{"database": "${record.database}", "table": "${record.table}", "name": "${record.name}"}, Confirm: true, ConfirmText: "Drop this column? Its data is permanently removed.", OnSuccess: &plugin.ActionSuccess{SelectTab: "columns"}, Bulk: true},
		...
		{ID: "mysql.index.drop", Label: "Drop index", Icon: icon("trash"), RouteID: "mysql.index.drop", Params: map[string]string{"database": "${record.database}", "table": "${record.table}", "name": "${record.name}"}, Confirm: true, ConfirmText: "Drop this index?", OnSuccess: &plugin.ActionSuccess{SelectTab: "indexes"}, Bulk: true},
		{ID: "mysql.table.row.delete", Label: "Delete row", Icon: icon("trash"), RouteID: "mysql.table.row.delete", Params: tableParams(), Body: map[string]any{"key": "${record._key}"}, Confirm: true, ConfirmText: "Delete selected row(s)? This cannot be undone.", VisibleWhen: &plugin.Condition{AllOf: []plugin.Rule{{Field: "_key", Op: plugin.OpNotEmpty}}}, Bulk: true},
		{ID: "mysql.table.truncate", Label: "Truncate", Icon: icon("eraser"), RouteID: "mysql.table.truncate", Params: tableParams(), Confirm: true, ConfirmText: "Truncate this table? Every row will be deleted.", Group: "Danger zone", Bulk: true},
		{ID: "mysql.table.drop", Label: "Drop table", Icon: icon("trash-2"), RouteID: "mysql.table.drop", Params: tableParams(), Confirm: true, ConfirmText: "Drop this table? The table definition and data will be permanently deleted.", OnSuccess: &plugin.ActionSuccess{Navigate: plugin.NavigateList}, Group: "Danger zone", Bulk: true},
		...
		{ID: "mysql.view.drop", Label: "Drop view", Icon: icon("trash-2"), RouteID: "mysql.view.drop", Params: map[string]string{"database": "${resource.namespace}", "view": "${resource.name}"}, Confirm: true, ConfirmText: "Drop this view?", OnSuccess: &plugin.ActionSuccess{Navigate: plugin.NavigateList}, Bulk: true},
		{ID: "mysql.database.drop", Label: "Drop database", Icon: icon("trash-2"), RouteID: "mysql.database.drop", Params: map[string]string{"database": "${resource.uid}"}, Confirm: true, ConfirmText: "Drop this database? All of its tables, views, and data will be permanently deleted.", OnSuccess: &plugin.ActionSuccess{Navigate: plugin.NavigateList}, Bulk: true},
		...
		{ID: "mysql.constraint.drop", Label: "Drop constraint", Icon: icon("trash"), RouteID: "mysql.constraint.drop", Params: map[string]string{"database": "${record.database}", "table": "${record.table}", "name": "${record.name}", "type": "${record.type}"}, Confirm: true, ConfirmText: "Drop this constraint?", OnSuccess: &plugin.ActionSuccess{SelectTab: "constraints"}, Bulk: true},
		...
		{ID: "mysql.user.drop", Label: "Drop user", Icon: icon("trash-2"), RouteID: "mysql.user.drop", Params: map[string]string{"user": "${resource.name}", "host": "${resource.namespace}"}, Confirm: true, ConfirmText: "Drop this user? Their privileges are permanently removed.", OnSuccess: &plugin.ActionSuccess{Navigate: plugin.NavigateList}, Group: "Danger zone", Bulk: true},
	}
}
