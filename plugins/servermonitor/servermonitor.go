...
func tabs() []plugin.Panel {
	return []plugin.Panel{
		...
		{Key: "processes", Label: "Processes", Icon: lucide("list-tree"), Type: plugin.PanelTable, Source: &plugin.DataSource{RouteID: "server_monitor.processes"}, Config: liveTableConfig(processColumns(), 10000, sortBy("cpuPct"))},
		...
		{Key: "connections", Label: "Connections", Icon: lucide("radio-tower"), Type: plugin.PanelTable, Source: &plugin.DataSource{RouteID: "server_monitor.connections"}, Config: liveTableConfig(connectionColumns(), 10000, nil)},
