...
func configSchema() plugin.Schema {
	...
	return plugin.Schema{Groups: []plugin.Group{
		{Name: "Server", Fields: []plugin.Field{
			{Key: "host", Label: "Host", Type: plugin.FieldText, Required: true, Placeholder: "clickhouse.example.internal"},
			...
			{Key: "database", Label: "Default database", Type: plugin.FieldText, Required: true, Default: "default"},
		}},
		{Name: "Authentication", Fields: []plugin.Field{
			...
			{Key: "username", Label: "Username", Type: plugin.FieldText, Required: true, Placeholder: "default", VisibleWhen: &passwordAuth},
		...
		{Name: "Safety", Fields: []plugin.Field{
			{Key: "read_only", Label: "Read-only mode", Type: plugin.FieldToggle, Default: true, Help: "Blocks INSERT, ALTER, DELETE, DDL, TRUNCATE, OPTIMIZE, SYSTEM, KILL, GRANT, and other write or operational statements."},
			{Key: "require_destructive_confirmation", Label: "Confirm destructive statements", Type: plugin.FieldToggle, Default: true, Help: "Requires explicit confirmation before writes, DDL, mutations, OPTIMIZE, SYSTEM, KILL, and privileged statements execute."},
...
func parseOptions(cfg plugin.ConnectConfig) (options, error) {
	...
	return options{
		...
		RowLimit:          rowLimit,
		MaxConns:          maxConns,
		ApplicationName:   "shellcn-clickhouse",
		RedactPatterns:    sqldb.ParsePatterns(cfg.String("redact_columns"), sqldb.DefaultRedactColumnPatterns()),
	}, nil
}

func driverOptions(opts options, netTransport plugin.NetTransport) (*clickhousedriver.Options, error) {
	...
	return &clickhousedriver.Options{
		Addr: []string{net.JoinHostPort(opts.Host, fmt.Sprint(opts.Port))},
		...
		ReadTimeout:     opts.QueryTimeout,
		MaxOpenConns:    opts.MaxConns,
		MaxIdleConns:    opts.MaxConns,
		ConnMaxLifetime: 30 * time.Minute,
		Settings: clickhousedriver.Settings{
			"max_execution_time": int(opts.QueryTimeout.Seconds()),
		},
		ClientInfo: clickhousedriver.ClientInfo{Products: []struct {
			Name    string
			Version string
		}{{Name: opts.ApplicationName, Version: "0.1.0"}}},
	}, nil
}
