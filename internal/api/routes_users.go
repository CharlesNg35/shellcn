...
func registerUserRoutes(api *gin.RouterGroup, handler *handlers.UserHandler, checker *permissions.Checker) {
	users := api.Group("/users")
	{
		...
		users.POST("/bulk/activate", middleware.RequirePermission(checker, "user.edit"), handler.BulkActivate)
		users.POST("/bulk/deactivate", middleware.RequirePermission(checker, "user.edit"), handler.BulkDeactivate)
		...
		users.PATCH("/:id", middleware.RequirePermission(checker, "user.edit"), handler.Update)
